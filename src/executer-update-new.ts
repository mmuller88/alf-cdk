  ...
  await Promise.all(
    event.Records.map(async (sqsRecord) => {
      ...
      if (
        ...
        JSON.stringify(newInstanceItem) !== '{}' &&
        JSON.stringify(oldInstanceItem) === '{}' &&
        newInstanceItem.expectedStatus === InstanceStatus.running
      ) {
        ...
        const params: CodeBuild.Types.StartBuildInput = {
          ...
          environmentVariablesOverride: [
            ...
            { name: 'stackName', value: newInstanceItem.instanceId },
      ...
      const expectedStatus =
        JSON.stringify(newInstanceItem) !== '{}' ? newInstanceItem.expectedStatus : InstanceStatus.terminated;

      const ec2params: EC2.Types.DescribeInstancesRequest = {
        Filters: [
          // { Name: 'instance-state-code', Values: ['16'] },
          {
            Name: `tag:${instanceTable.instanceId}`,
            Values: [
              JSON.stringify(newInstanceItem) !== '{}' ? newInstanceItem.instanceId : oldInstanceItem.instanceId,
            ],
          },
        ],
      };
          ...
          if (expectedStatus === InstanceStatus.terminated) {
            const startBuildInput: CodeBuild.Types.StartBuildInput = {
              ...
              environmentVariablesOverride: [
                {
                  name: 'CDK_COMMAND',
                  value: `aws cloudformation delete-stack --stack-name ${oldInstanceItem.instanceId} --region ${CFN_REGION} --profile damadden88`,
                },
              ],
            };
